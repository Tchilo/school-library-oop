...
class Persistor
  ...
  def persist(people:, rentals:, books:)
    File.open('people.json', 'w') { |f| f.write JSON.generate(people) } unless people.empty?# rubocop:todo Lint/UselessAssignment
    File.open('books.json', 'w') { |f| f.write JSON.generate(books) } unless books.empty?# rubocop:todo Lint/UselessAssignment
    File.open('rentals.json', 'w') { |f| f.write JSON.generate(rentals) } unless rentals.empty?# rubocop:todo Lint/UselessAssignment
  end
end
